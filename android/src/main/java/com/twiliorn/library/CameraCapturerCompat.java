...
public class CameraCapturerCompat {
    ...
    public CameraCapturerCompat(Context context, CameraCapturer.CameraSource cameraSource, final CameraSwitchedListener listener) {
        if (Camera2Capturer.isSupported(context) && isLollipopApiSupported()) {
            ...
            CameraCapturer.Listener cameraListener =
...
//        for (camera2Enumerator.getDeviceNames())
        for (String cameraId : camera2Enumerator.getDeviceNames()) {
            if (isCameraIdSupported(cameraId)) {
                if (frontCameraPair == null && camera2Enumerator.isFrontFacing(cameraId)) {
                    frontCameraPair =
                            new Pair<>(CameraCapturer.CameraSource.FRONT_CAMERA, cameraId);
                }
                if (backCameraPair == null && camera2Enumerator.isBackFacing(cameraId)) {
